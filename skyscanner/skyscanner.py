...
try:
    ...
    import config
    from px import PXSolver
    from types_skyscanner import ( # Fallback if local
        Location,
        Airport,
        CabinClass,
        SpecialTypes,
...
class SkyScanner:
    ...
    def get_flight_prices(
        ...
        if req.status_code == 403:
            print(f"DEBUG: 403 Response at get_flight_prices: {req.text}")
            try:
                redirect = req.json().get("redirect_to", "")
            except:
                redirect = ""
            raise BannedWithCaptcha(
                "https://www.skyscanner.net" + redirect
            )
    ...
    def search_airports(
        ...
        if req.status_code == 403:
            print(f"DEBUG: 403 Response at search_airports: {req.text}")
            try:
                redirect = req.json().get("redirect_to", "")
            except:
                redirect = ""
    ...
    def search_locations(self, query: str) -> list[Location]:
        ...
        if req.status_code == 403:
            print(f"DEBUG: 403 Response at search_locations: {req.text}")
            try:
                redirect = req.json().get("redirect_to", "")
            except:
                redirect = ""
    ...
    def get_itinerary_details(
        ...
        if req.status_code == 403:
            print(f"DEBUG: 403 Response at get_itinerary_details: {req.text}")
            try:
                redirect = req.json().get("redirect_to", "")
            except:
                redirect = ""
