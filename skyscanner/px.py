...
class PXSolver:
    ...
    def gen_px_authorization(self):
        print("DEBUG: Starting gen_px_authorization...")
        fingerprint = self.__get_fingerprint()
        print(f"DEBUG: Using Fingerprint: {fingerprint['model']} ({fingerprint['brand']})")
        auth, uuid = self.gen_px(fingerprint)
        print(f"DEBUG: Generated Auth: {auth[:20]}... UUID: {uuid}")
        return auth, uuid
    ...
    def gen_px(self, fingerprint):
        ...
        if first_req.status_code != 200:
            print(f"DEBUG: PX First Request Failed. Code: {first_req.status_code}, Response: {first_req.text}")
            raise PerimeterXError(f"Error while posting first payload, code {first_req.status_code} message : {first_req.text}")
        
        print(f"DEBUG: PX First Request Success. Response: {first_req.text[:100]}...")
